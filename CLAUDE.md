...
- **說話者分離靠聲紋（`src/offscreen/voiceprint.js`），不靠畫面。**
  ...
  **沒有這張對照表的話，姓名的覆蓋率就等於字幕的覆蓋率**，而那很低。
  ...
  實作上的取捨：MFCC 的**平均值 ＋ 標準差**（標準差帶的是語調與共振峰的
  移動幅度，比平均值更能分開音色相近的人）、只算**有聲音的訊框**
  （靜音佔比不同會讓同一個人被判成兩個人）、只取 **300–3400 Hz**
  （會議音訊被編碼器壓過，範圍外的資訊不可靠）。各維度要**先除以經驗尺度
  再 L2 正規化** —— 不然餘弦相似度幾乎只反映第一個係數，所有人都是 0.99。
  `threshold` 是唯一要調的參數，**寧可偏高**：「講者 1 和 3 其實是同一個人」
  看得出來，「兩個人混在同一個標籤下」看不出來，而且會把摘要帶歪。

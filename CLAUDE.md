...
| 看畫面 | Claude Code 橋接（唯一選項） | 10–30 秒 | 沒有 —— 雲端沒有視覺模型 |
| 查證 | Tavily（只在需要時） | 1–2 秒 | 沒有就跳過，不擋住回答 |

### 不要憑記憶寫模型 ID —— 先跑 `tools\list-groq-models.ps1`

**踩過一次，而且是很難發現的那種。** 想讓「附上會議畫面」不必依賴橋接，
於是把 `supportsImages` 改成 true 並加了一條 `vision` 候選鏈，填的是
`meta-llama/llama-4-scout-17b-16e-instruct` 與 `…-maverick-…`。
**兩個都回 404。** 實際去問 `/models` 才發現 Groq 這個帳號的 15 個模型裡
**一個視覺模型都沒有**：兩支 whisper、llama-3.1/3.3、gpt-oss、qwen3.6、compound。

為什麼難發現：**404 在畫面上跟「額度用完」長得一模一樣** —— 都是
「雲端後端暫時不可用，改用 Claude Code」，使用者只會覺得今天特別慢。
而合成測試用的是 stub，根本不檢查模型名稱，所以全綠。

所以：

- 要改候選鏈**先跑 `powershell -File tools\list-groq-models.ps1`** 看實際清單。
- `cloud.js` 收到 **404 會冷卻 300 秒**（跟 401 一樣），因為那不是「等一下就好」
  而是「這個名稱不存在」—— 不冷卻的話每次呼叫都白撞一遍整條鏈。
- 「看畫面」仍然只有 Claude Code 橋接做得到。

**橋接的脆弱點修掉了。** `bridge/config.json` 存的是 `claude.exe` 的**絕對路徑**，
而 Claude Code 的 VS Code 擴充功能會自動更新到新的版號資料夾
（`anthropic.claude-code-2.1.228-…`），舊路徑就失效。使用者看到的是
「勾了附上會議畫面就出錯，不勾就正常」，完全聯想不到是**別的軟體更新**造成的，
而且每次更新都會再發生一次。`host.ps1` 現在在路徑不存在時會自己重新找一次
（跟 `install.ps1` 同樣的邏輯）並寫回 config.json。
...
- **「有人在問我」要看前後一小段對話，不能只看一個段落。**
  逐字稿改成一句一段之後**踩到迴歸**：原本會自動回答的問題全部不再觸發。
  原因是人不會把名字和問題塞進同一句話 ——
  「小陳，」→「成本優勢明顯，」→「這個是在哪裡體現？」名字在第一段、
  問句在第三段。切細之前一整塊 28 秒是**一個**段落，兩者剛好都在裡面，
  所以舊寫法（只檢查 `segment.text`）能過純粹是巧合。
  現在 `findAddressing()` 會往回看 20 秒。

  去重用的是「**哪一段話點了我**」的 id，不是時間。一開始寫成「12 秒內不再
  回答」，但那樣真正的追問會被吃掉，而且行為綁在牆上時鐘 ——
  測試只能靠 sleep 或去改模組內部狀態才驗得到（而模組內部的 `let`
  在測試環境是碰不到的，改到的只是一個同名的全域變數，看起來有效其實沒有）。
- **說話者分離靠聲紋（`src/offscreen/voiceprint.js`），不靠畫面。**
  這是實測後的結論：讀 DOM 的兩條路（字幕、發言指示器）都有致命弱點 ——
  字幕要使用者自己開，而**看共享畫面時 Teams／Meet 根本不顯示視訊磚**，
  指示器與名牌一起消失。偏偏那正是最需要逐字稿的場合。

  所以現在的分工是：**聲紋回答「這是不是同一個人」，字幕／名牌回答
  「這個人叫什麼」。** offscreen 只送分群 id（`cluster`），
  名字的判斷全部留在背景的 `resolveSpeakerName()`。

  關鍵在 `clusterNames`：只要在**任何一刻**替某一群對上過一次真名，
  之後那一群的每一段都顯示真名 —— 即使字幕早就斷了。
  **沒有這張對照表的話，姓名的覆蓋率就等於字幕的覆蓋率**，而那很低。

  判斷條件是 **AND：基頻要接近，而且去掉共同成分的 MFCC 要相似。**
  兩個都通過才併群，所以偏向拆開 —— 「講者 1 和 3 其實是同一個人」看得出來，
  「兩個人混在同一個標籤下」看不出來，而且會把摘要帶歪。

  **兩個踩過的坑，都是「合成測試全綠但真實會議整場併成講者 1」：**

  1. **只用 MFCC 餘弦配固定門檻是行不通的。** 合成的兩個聲音共振峰差很遠
     （實測相似度 0.32 對 0.97），任何門檻都分得開。但真人不是這樣：
     同一個語言、同一支麥克風、同一個編碼器壓過的兩個人，MFCC 平均值裡
     **共同成分遠大於個人成分**，不同人也有 0.95 以上。所以一定要先減掉
     整場的移動平均（cepstral mean normalization）再比，而且要加上**基頻**
     這個絕對量 —— 它不受編碼器與語言影響。
     **合成測試要讓兩個聲音共用大部分共振峰骨架**，不然它證明不了任何事。
  2. **CMN 在前幾段是數學上壞掉的。** 減數是整場平均，只有兩段時那個平均
     正好是兩者的中點，於是兩個殘差方向相反、**餘弦恆等於 -1**，
     同一個人的第二句話一定被判成新講者。所以 `seen < 6` 時只用基頻，
     等統計量有意義了才把 MFCC 加進來。

  其他取捨：MFCC 取**平均值 ＋ 標準差**（標準差帶的是語調與共振峰的移動幅度，
  比平均值更能分開音色相近的人）、只算**有聲音的訊框**（靜音佔比不同會讓
  同一個人被判成兩個人）、只取 **300–3400 Hz**（編碼器壓過，範圍外不可靠）、
  基頻取**中位數**（總有幾個訊框會抓到泛音，平均會被拉走）。
  算不出來時回 `null`／`0` 而不是亂猜的向量 —— 亂猜會污染分群中心，
  錯一次之後整場都跟著錯。
- **參與者名單是聲紋分群的硬上界，而且一對一時根本不需要聲紋。**
  實測回報過「實際只有兩個人講話，逐字稿卻標出講者 1／2／3」——
  分群自由生長的結果。同一個人的音高在不同句子之間本來就會起伏，
  偶爾就超過門檻開了新的一群，而**兩個人的會議不可能有三個講者**。
  所以 `updateRoster()` 會把「我以外的人數」送給 offscreen 當
  `maxSpeakers`（`ma:offscreen:speakers`）。

  更重要的是：**我以外只剩一個人時，所有不是我說的話都是他說的** ——
  直接用他的名字，不必問聲紋、不必等字幕、不必有發言指示器。
  那是最可靠的一條路，`resolveSpeakerName()` 第一件事就是檢查它。

  `speakerMax` 要**跟 speakerBook 分開存**：名單通常在開始聆聽之前就送來了，
  而 `speakerBook` 是第一次算聲紋時才建立、`stop()` 會清掉 ——

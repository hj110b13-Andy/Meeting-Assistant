...
function nameHint() {
  ...
  // 分群已經在做了（「講者 1／2」），缺的只是名字。名字有兩個來源：
  // 畫面上的名牌（切到共享畫面時就沒有了）與字幕。都沒有才會走到這裡。
  p.textContent = '「講者 1／2／3」是靠聲音分出來的 —— 同一個人永遠是同一個編號，不需要開字幕。'
    + '但聲音裡沒有姓名，所以要換成真名還是得有字幕或畫面上的名牌。'
    + (steps ? `在會議裡開啟字幕就會換成真名，而且只要對上一次就會一直記住（之後字幕關掉也沒關係）：${steps}。`
      : '在會議裡開啟即時字幕就會換成真名，而且只要對上一次就會一直記住。')
    + '（你自己的發言一律標成「我」，不需要這些。）';
  return p;
}

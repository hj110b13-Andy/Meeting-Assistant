 ...
 * 「為什麼看不到真實姓名」的說明。
 *
 * 現在說話者會顯示成「講者 1／2／3」（聲紋分群，不需要字幕也分得開），
 * 所以逐字稿本身已經讀得懂 —— 缺的只是「講者 2 到底叫什麼」。
 * 而那個資訊聲音裡沒有，只有會議自己的字幕或畫面上的名牌有。
 *
 * 只在真的用得上的時候出現：正在聽、已經有逐字稿、而且**一個真名都沒有**。
 * 對上任何一個真名就表示已經有姓名來源了，這時再提醒只是雜訊。
 */
const PLACEHOLDER_SPEAKER = /^(其他人|講者\s*\d+)/;
...
function nameHint() {
  ...
  // 姓名有兩個來源：畫面上的發言指示器（不必開字幕）與字幕本身。
  // 兩個都沒給出名字才會走到這裡，所以要講的是「還可以怎麼救」。
  // 分群已經在做了（「講者 1／2」），缺的只是名字。名字有三個來源，
  // 由可靠到不可靠：參與者名單只剩一個人（直接就是他）、畫面上的名牌
  // （切到共享畫面時就沒有了）、字幕。都沒有才會走到這裡。
  p.textContent = '「講者 1／2／3」是靠聲音分出來的 —— 同一個人永遠是同一個編號，不需要開字幕。'
    + '但聲音裡沒有姓名，所以要換成真名得從會議畫面上讀 —— 目前讀不到（例如參與者清單沒開、或正在看共享畫面）。'
    + (steps ? `在會議裡開啟字幕最可靠，而且只要對上一次就會一直記住（之後字幕關掉也沒關係）：${steps}。`
      : '在會議裡開啟即時字幕最可靠，而且只要對上一次就會一直記住。')
    + '（你自己的發言一律標成「我」，不需要這些。）';
  return p;
}
...
function renderTranscript() {
  ...
  if (!rows.length) {
    ...
    p.textContent = q
      ? '沒有符合的段落。'
      : '等待發言…開始說話後幾秒會出現逐字稿。說話者會先標成「講者 1／2」（靠聲音分的），在會議裡開啟字幕就會換成真名。';
    box.appendChild(p);
  } else {
    const hint = nameHint();

...
| **重點** | 自動更新需要**兩個條件同時成立**：累積至少 N 段發言**且**距上次至少 M 秒（預設 8 段 / 300 秒，可在設定頁調整）。等不及就按 **✦ 產生重點**。 |
| **問答** | 自動偵測的提問 + 你手動輸入的問題。每張卡片可一鍵複製。 |

**✦ 產生重點** 立刻整理一份，不等上面那兩個門檻。按下去之後**自動摘要的段數與
時間基準會一起重算**，所以不會出現「才剛手動產生完，一分鐘後又自動跑一次」。
沒有逐字稿的時候這顆是停用的。

這顆刻意保留成按鈕（其他動作都改成自動了），因為它對應的是一個明確的當下意圖 ——
「我現在就想要一份」，例如剛講完一個段落、或要在會議中途對齊進度。
自動摘要不可能猜到那個時間點。

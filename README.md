...
**語音辨識完全不做說話者分離。** whisper 回來的就是一整段文字,沒有任何
「這句是誰講的」資訊,三個引擎都一樣。所以拆成兩個問題,各自用不同的方法:

| 問題 | 用什麼 | 需要開字幕嗎 |
|---|---|---|
| **這是不是同一個人？** | **聲紋分群**（`src/offscreen/voiceprint.js`） | 不需要 |
| **這個人叫什麼？** | 參與者名單 ／ 畫面上的名牌 ／ 會議字幕 | 至少一次 |

**參與者名單是分群的硬上界**：兩個人的會議不可能有三個講者。
實測回報過「實際只有兩個人講話卻標出講者 1／2／3」—— 那是分群自由生長的
結果（同一個人的音高在不同句子之間本來就會起伏，偶爾就開了新的一群）。
...
而**只要在任何一刻對上過一次真名,那一群之後都顯示真名** ——
即使字幕後來斷掉,或畫面切到共享畫面沒有視訊磚了。這比單靠字幕穩定得多:
沒有這張對照表的話,姓名的覆蓋率就等於字幕的覆蓋率,而那個覆蓋率很低。

#### 為什麼不能只靠讀畫面

一開始是這樣做的（讀 Meet／Teams 的發言指示器），實測後發現致命弱點:
**在看共享畫面的時候,Teams 與 Meet 根本不顯示視訊磚** ——
沒有磚就沒有指示器也沒有名牌。而「有人在分享畫面」正是最需要逐字稿的場合。

字幕也不可靠:它要使用者自己在會議裡開啟,很容易漏掉,而且斷斷續續。

聲紋只看聲音,上面那些情況全都不影響它。

#### 聲紋是怎麼算的

VAD 已經把音訊切成「一句一段」,每段幾乎都只有一個人在講 —— 所以剩下的
只是「這一段跟前面哪一段是同一個人」。那用 **MFCC 統計量 ＋ 線上分群**
就夠了,完全在瀏覽器裡跑、零依賴、不下載任何模型。

判斷條件是 **AND:基頻要接近,而且去掉共同成分的頻譜特徵要相似。**
兩個都通過才算同一個人,所以整體偏向拆開。

#### 兩個踩過的坑（都是「測試全綠但實際整場併成講者 1」）

**① 只用頻譜相似度配一個固定門檻,行不通。**
合成的兩個聲音共振峰差很遠（實測相似度 0.32 對 0.97）,任何門檻都分得開。
但真人不是這樣:同一個語言、同一支麥克風、同一個編碼器壓過的兩個人,
**共同成分遠大於個人成分**,不同人也有 0.95 以上的相似度。

所以一定要先減掉整場的移動平均（語音辨識裡叫 cepstral mean normalization）
再比,而且要加上**基頻**這個絕對量 —— 它不受編碼器與語言影響,
而且是最強的廉價線索（男聲約 85–155 Hz、女聲約 165–255 Hz）。

**這也是測試該學到的事:合成的兩個聲音必須共用大部分共振峰骨架**,
不然那個測試證明不了任何東西。

**② 減平均這件事在前幾段是數學上壞掉的。**
減數是「整場的平均」,而只有兩段時那個平均正好是兩者的中點 ——
於是兩個殘差方向剛好相反,**餘弦相似度恆等於 −1**,不管是不是同一個人。
實測就是這樣:同一個人的第二句話得到 −1.000,被判成新的講者。

所以前 6 段只用基頻判斷（它不需要任何整場統計就站得住）,
等統計量有意義了才把頻譜特徵加進來。

#### 其他取捨

- **用平均值 ＋ 標準差**,不只用平均值。標準差帶的是「這個人講話時頻譜
  怎麼變化」（語調、氣息、共振峰的移動幅度）,比平均值更能分開音色相近的人。
- **只算有聲音的訊框。** 一段話裡的靜音會把平均值往「無聲的頻譜」拉,
  而每個人的靜音聽起來都一樣 —— 靜音佔比不同的兩段話會因此被判成不同的人。
- **只取 300–3400 Hz。** 會議音訊被各家平台的編碼器壓過,那個範圍以外的
  資訊本來就不可靠。
- **基頻取中位數,不取平均。** 一段話裡總有幾個訊框會抓到泛音（頻率翻倍）,
  平均值會被那些離群值拉走。
- **門檻寧可偏向拆開。** 偏向拆開的症狀是「講者 1 和講者 3 其實是同一個人」,
  那看得出來;偏向合併的症狀是「兩個人的話混在同一個標籤下」,那看不出來,
  而且會把摘要與回答建議一起帶歪。
- **算不出來時回「不知道」,不回一個亂猜的向量。** 亂猜會污染分群中心,
  錯一次之後整場都跟著錯。

> **限制**:合成音訊的測試只能證明「演算法會把不同音色分開」,
> 不能證明真人的聲音一定分得開。音色很接近的兩個人、或同時講話的段落
> 仍然會混在一起。門檻在 `voiceprint.js` 的 `threshold`。

發言指示器這條有三道防線,**每一道都是為了「寧可沒有名字,也不要掛錯人」**:
名字要像名字(不能是 `arrow_drop_down` 這種圖示名稱)、要對得上參與者名單、
**同時偵測到兩個人就整個丟掉**(那時候沒辦法知道這段音訊是誰的)。

往上爬找名字時最危險的一步是爬到「裝著所有人視訊磚的格線」那一層 ——
在那裡取第一個名字會讓**所有人的話都掛到畫面上第一個人頭上**。所以條件是
「這一層只有一個名字」,那等於「這一層就是某一個人的磚」,不依賴任何 class。

> **指示器的選擇器抓不到的話**,在會議分頁的 console 跑 `__MA_SPEAKER_DEBUG__()`
> 會印出每一條候選選擇器的命中數,以及目前偵測到誰在講話。
> 設定頁的「診斷」也會印出命中了哪一條。改選擇器要照著這個結果改,不要猜。

記**區間**而不是時間點是必要的:一則字幕會持續好幾秒(而且會被串流改寫、
同一個 id 定稿多次)。只記一個時間點的話,比對變成「離哪一點最近」——
一句講了 8 秒的話,後半段可能離下一個人的起點更近,於是被安到**還沒開口的人**
頭上。容許誤差是 **±6 秒**(平台產生字幕的延遲實測 1–3 秒,留了兩倍餘裕)。
超出就寧可顯示佔位標籤 —— 掛錯人在畫面上看起來完全正常,只有當事人自己
看得出不對,而摘要與回答建議已經被帶歪了。

#### 為什麼要逐句的時間戳

雲端這條為了守住 20 RPM 會把相鄰音訊**合併到 28 秒**,而 28 秒裡換三次人
是很正常的。整塊只有一個時間點可比對的話,三個人講的話會全部掛在同一個
人頭上。

所以雲端辨識用 `response_format=verbose_json` ＋
`timestamp_granularities[]=segment`,讓 whisper 把它自己切出來的每一句都附上
相對秒數;加上這塊音訊的起始時間就是每一句的**絕對說話時間**,於是每一句
各自對上正確的人:
...
**這不會多花額度也不會變慢** —— 同一次請求,只是要求更詳細的回應格式。
拿不到 `segments` 時(換模型、API 改版)會自動退回整段文字:逐字稿仍然完整,
只是姓名的粒度變粗。`run-cloud-check.ps1` 有一項打真 API 驗這個欄位還在,
因為合成測試裡 segments 是我們自己塞的,欄位消失時合成測試照樣全綠。

> **沒開字幕就沒有真名,沒有別的辦法。** 免費方案裡沒有能分辨人聲的模型
> (那是 diarization,要另一套模型)。所以側邊欄在「有逐字稿但一個真名都沒有」
> 的時候會直接說明原因,並給出**你這個平台**開字幕的具體步驟。
> 你自己的發言一律標成「我」,那條走麥克風,不需要字幕。
